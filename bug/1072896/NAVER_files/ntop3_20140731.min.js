...
var c=b.length;if(c&&c>0){var a=0;this.rollingStop();this.interval=setInterval(function(){$Element(b[a++]).hide();if(a>=c){a=0}var d=$Element(b[a]);d.show("block");$$.getSingle("#ts_region strong").innerHTML="국내<span class='dep2'> &gt; </span>"+d.attr("data-name")
},5000)}},rollingStop:function(){clearInterval(this.interval)},_setPage:function(){var a=$$.getSingle(".tsq_weather",TS.pan);var c=this.level2;if(!c.code){if($Element(a).child().length==0){if(TS.LifeWeather.data.defaultHTML){a.innerHTML=TS.LifeWeather.data.defaultHTML;
this.rollingStart()}else{var b=$Ajax("/include/timesquare/weatherLife/default.html",{timeout:3,onload:function(e){TS.LifeWeather.data.defaultHTML=e.text();a.innerHTML=TS.LifeWeather.data.defaultHTML;TS.LifeWeather.rollingStart()
},onerror:function(){TS.failMsg("timesquare ajax error : /include/timesquare/weatherLife/default/html")}});b.header({"If-Modified-Since":"Thu, 1 Jan 1970 00:00:00 GMT"});b.request()}}else{this.rollingStart()
}}else{$$.getSingle("#ts_region strong").innerHTML=this.level0.name+"<span class='dep2'> &gt; </span>"+(this.isWorld()?this.level2.name:this.level3.name);if(this.isWorld()){var b=$Ajax("/include/timesquare/weatherLife/"+c.code+".html",{timeout:3,onload:function(e){a.innerHTML=e.text()
...
if(a==null){$Element(this.bannerName).show()}else{$Element(this.bannerName).hide()}},setCookie:function(){$Cookie().set(this.cookieName,"1",this.cookieExpire)}});var tsp=$("promotion2");if(tsp!==null){var bannerPlate=new TimeSquarePromotion("promotion2","promotion2Close","promotion2Link",tspCookieName,tspCookieExpDate,tspOneTimeLink);
...
break}},selectCategory:function(e,f,a){var b=this;if(!f){f=(!global.isSupportedFlicking)?this.onLoadPage:this.Flicking.loadBothCategory}var d=this.cacheData[e];if(d){return f(e,d,a)}var c="/nvtheme_body?theme="+e;
...
}}).request()},showConfig:function(){var a=this;var c=$Cookie().get("NM_THEMECAST_USER");var d=c?c:"tcc_non";var b="/include/themecast/menu_config.html";if(a.configLayer.child().length>0){a.configLayer.show();
$("config_"+d).checked=true}else{$Ajax(b,{timeout:3,onload:function(e){a.configLayer.html(e.text());a.configLayer.show();$("config_"+d).checked=true},onerror:function(){JEagleEyeClient.sendError("[THEMECAST] ajax fail(onerror) - url : "+b)
...
$Element("themecast_contents").delegate("click",".tc_bottom .tc_set",this.onclickConfig);$Element("themecast_contents").delegate("click",".area_book h4 a",this.onclickBookMenu);$Element("themecast_contents").delegate("click","a:not([href^=#])",this.onclickContentsLink);
$Element("theme_config").delegate("click",".cast_set_lst li input",this.onclickRadio);$Element(document).attach("click",this.onclickDocument)},onclickCategory:function(b){b.stop();var c=$Element(b.element);
if(c.hasClass("on")){return}var d=$Element(b.element).attr("data-code");TC.selectCategory(d);var a="tct."+d.replace("tcc_","");clickcr(b.element,a,"","",b._event);TC.UI.showFlickingNoti()},onclickPrev:function(c){c.stop();
var d=TC.cateArr.indexOf($Element($$.getSingle("#themecast_cate .on")).attr("data-code"));var b=(d<=0)?(TC.cateArr.length()-1):(d-1);var a=TC.cateArr.get(b);TC.selectCategory(a);clickcr(c.element,"tct.prev","","",c._event);
TC.UI.showFlickingNoti()},onclickNext:function(b){b.stop();var c=TC.cateArr.indexOf($Element($$.getSingle("#themecast_cate .on")).attr("data-code"));var d=(c>=(TC.cateArr.length()-1))?0:(c+1);var a=TC.cateArr.get(d);
TC.selectCategory(a);clickcr(b.element,"tct.next","","",b._event);TC.UI.showFlickingNoti()},onclickConfig:function(a){a.stop();TC.showConfig();clickcr(a.element,"tcg.set","","",a._event)},onclickBookMenu:function(b){b.stop();
var a=$Element(b.element);$ElementList("#themecast_contents .area_book h4 a").removeClass("on");$ElementList("#themecast_contents .book_new, #themecast_contents .book_best").hide();a.addClass("on");var c=a.attr("data-nclk");
if(c==".new"){$Element($$.getSingle("#themecast_contents .book_new")).show()}else{if(c==".best"){$Element($$.getSingle("#themecast_contents .book_best")).show()}}clickcr(b.element,c,"","",b._event)},onclickContentsLink:function(a){var b=TC.Cate.getCurrentCode();
$Cookie().set("NM_THEMECAST_LAST",b,1/24/60*30);if(b=="tcc_occ"){var c=TC.Opencast.curCode;var d=TC.Opencast.curPage;$Cookie().set("NM_OPENCAST_LAST",c+","+d,1/24/60*30)}},onclickDocument:function(a){TC.Opencast.Layer.leaveAll()
},onclickRadio:function(a){var b=a.element.id;clickcr(a.element,"tcg."+b.replace("config_tcc_",""),"","",a._event)}},Opencast:{cache:{},template:{prefix:'<div class="opc"><h4 class="opc_tit"><a href="http://opencast.naver.com/{=id}/{=seq}" onclick="return clickcr(this,\'{=nclkArea}.title\',\'\',\'\', event);">{=volumeTitle }</a></h4><div class="opc_origin"><a href="http://opencast.naver.com/{=id}" onclick="return clickcr(this,\'{=nclkArea}.cast\',\'\',\'\', event);">{=title }</a><em> No.{=seq }</em><span class="bar"> | </span><a href="#" class="rd" data-cast-id="{=id}"><span></span> 구독</a></div>',"1":'<div class="opc_type_th"><div class="th"><a href="{=articles[0].url }" onclick="return clickcr(this,\'{=nclkArea}.contents\',\'\',\'\', event);"><span class="thumb"><img src="{=articles[0].thumbnailUrl}" width="142" height="83" alt="{=articles[0].title }"><span class="mask"></span></span><span class="t">{=articles[0].title }</span></a></div><ul>{for idx:article in articles}{if idx != 0}<li><a href="{=article.url }" onclick="return clickcr(this,\'{=nclkArea}.contents\',\'\',\'\', event);">{=article.title }</a></li>{/if}{/for}</ul></div>',"3":'<ul class="opc_type_lst">{for idx:article in articles}<li><a href="{=article.url }" onclick="return clickcr(this,\'{=nclkArea}.contents\',\'\',\'\', event);"><span class="thumb"><img src="{=article.thumbnailUrl }" width="142" height="83" alt="{=article.title }"><span class="mask"></span></span><span class="t">{=article.title }</span></a></li>{/for}</ul>'},ERR_SERVER_CHECKING:-8,ERR_UNSUBSCRIBE_MYCAST:-3,HTML_SERVER_CHECKING:'<div class="opc_alert"><span class="ic_alert"></span><p class="oa1">MY캐스트 서버 점겅중입니다.</p><p class="oa2">서버 점검으로 일시적으로 MY캐스트 목록을 가져올 수 없습니다.<br>잠시 후에 다시 시도해 주세요.</p></div>',HTML_SERVER_ERROR:'<div class="opc_alert"><span class="ic_alert"></span><p class="oa3">일시적인 장애로 캐스트 목록을 가져올 수 없습니다.</p><p class="oa2">잠시 후에 다시 시도해 주세요.</p></div>',emptyCastHTML:'<div class="opc"><div class="opc_subs"><a href="http://opencast.naver.com/"><span class="ic_subs"></span><p>더 다양한 캐스트를 구독해보세요</p></a></div></div>',SIZE:4,init:function(){if(this.isInited){return
...
}var a=this.getLastPage(b);return(c>a)?true:false},getLastPage:function(b){var a=null;if(b=="S99"){return Math.ceil(this.cache.myCastList.length/this.SIZE)}else{return Math.floor(this.cache.categoryList[b].casts.length/this.SIZE)
...
}}).request();a.Layer.leave("opc_layer_unsubs")},EventManager:{init:function(){$Element("themecast_contents").delegate("click",".opc_cate a[data-code]",this.onclickCategory);$Element("themecast_contents").delegate("click",".opg a.pre",this.onclickPrev);
$Element("themecast_contents").delegate("click",".opg a.next",this.onclickNext);$Element("themecast_contents").delegate("click",".rd",this.onclickSubscribe);$Element("themecast_contents").delegate("click",".del",this.onclickUnSubscribe)
...
},move:function(b){if(this.data==null){var a=$Ajax("/include/question/question.json.10",{onload:function(c){if(c.status()==200){QST.data=c.json();QST.setIndex(b)}},onerror:function(c){LogError("[QUESTION] ajax data loading failed")
}});a.header("If-Modified-Since","Thu, 1 Jan 1970 00:00:00 GMT");a.request();a=null}else{this.setIndex(b)}},setIndex:function(a){if(a){if(this.index==this.data.length-1){this.index=0}else{this.index++}}else{if(this.index==0){this.index=(this.data.length-1)
}else{this.index--}}this.changeData()},changeData:function(){var e=$Element("qu_txt");var a=this.data[this.index].length;this.detachClickcr();e.empty();for(var d=0;d<a;d++){var b=this.data[this.index];
var c=[];if(d==0){c.push("<span>")}else{c.push('<span class="e">')}c.push('<a href="');c.push(b[d].url);c.push('" title="');c.push(b[d].txt);c.push('" name="');c.push(b[d].gdid);if(this.data[this.index][d].new_yn=="Y"){c.push('" target="_blank"')
}c.push('">');c.push(b[d].txt);c.push("</a></span>\n");e.$value().innerHTML+=c.join("")}this.attachClickcr(e);this._setPageNum()},detachClickcr:function(){var b=$$("#qu_txt a");for(var a=0;a<b.length;a++){this.click.detach(b[a],"click");
b[a].onclick=null}},attachClickcr:function(b){var c=$$("#qu_txt a");for(var a=0;a<c.length;a++){this.click.attach(c[a],"click")}},setClickcr:function(a){var b=a.currentElement;clickcr(a.element,"txt.search",b.name,"",a._event)
},_setPageNum:function(){if(!this.elPageNum){return}this.elPageNum.innerHTML="<strong>"+(this.index+1)+"</strong>/"+this.size}};var QST=Question;var SlidePromotion={init:function(a,d,b,c){this.element=$Element(a);
this.closeButton="promotion8_close";this.cookieName=d;this.cookieExpire=b;this.clickLinkAndClose=c;this.heightNow=0;this.heightUnit=3;this.heightLimit=-31;this.slidingStatus=null;if($(this.closeButton)){$Fn(this.onCloseBanner,this).attach(this.closeButton,"click")
}this.checkBanner()},checkBanner:function(b){var a=$Cookie().get(this.cookieName);if(a==null){this.show()}else{this.hide()}},onCloseBanner:function(a){this.hide();this.setCookie();clickcr(a.element,"tsb.close","","",a._event,1);
a.stop()},onClickBanner:function(a,b){clickcr(a,"tsb.link",this.cookieName,"",b);if(slidingOneTimeLink=="1"){this.setCookie()}},setCookie:function(){$Cookie().set(this.cookieName,"1",this.cookieExpire)
},hide:function(){this.element.hide()},show:function(){this.element.css({top:[this.heightLimit,"px"].join("")});this.element.show();this.slide()},slide:function(){var a=this.heightLimit+this.heightNow;
if(a>=0){a=0}this.element.css({top:[a,"px"].join("")});if(a==0){clearTimeout(this.slidingStatus);this.slidingStatus=setTimeout("SlidePromotion.fadeout();",12000)}else{this.heightNow+=this.heightUnit;this.slidingStatus=setTimeout("SlidePromotion.slide();",30)
}},fadeout:function(){var a=this.heightLimit+this.heightNow;if(a<=this.heightLimit){a=this.heightLimit}this.element.css({top:[a,"px"].join("")});if(a==this.heightLimit){clearTimeout(this.slidingStatus);
this.slidingStatus=null}else{this.heightNow-=this.heightUnit;this.slidingStatus=setTimeout("SlidePromotion.fadeout();",30)}}};var RightBanner={init:function(){var a=$Element("promotion11");if(a==null||a.attr("data-rolling")!="on"){return
}this.items=$$("#promotion11 a");if(this.items.length<=2){return}var b=this;this.roll=new Rolling();this.roll.touch("promotion11",{direction:1,size:(b.items.length-1)*40,moveSize:40,moveStep:0.1,moveInterval:30,effect:function(c){var c=Math.sin(c*(Math.PI/2));
return c*c*c*c},freeze:5000})}};CenterBanner={init:function(){var e=44;var a=$Element("promotionElection");if(a&&a.hasClass("prmt_election2")){e=53}var b=$$("#rate_rolling ul li");if(b.length>1){var d=$Element(b[0]).outerHTML();
$Element(b[b.length-1]).afterHTML(d);this.rateRoll=new Rolling();this.rateRoll.touch("rate_rolling",{direction:1,size:e*b.length,moveSize:e,moveStep:0.1,moveInterval:30,effect:function(f){var f=Math.sin(f*(Math.PI/2));
return f*f*f*f},freeze:3000})}var c=$$("#prmt_rolling ul li");if(c.length<2){c=$$("#prmt_rolling ul")}if(c.length>1){var d=$Element(c[0]).outerHTML();$Element(c[c.length-1]).afterHTML(d);this.prmtRoll=new Rolling();
this.prmtRoll.touch("prmt_rolling",{direction:1,size:e*c.length,moveSize:e,moveStep:0.1,moveInterval:30,effect:function(f){var f=Math.sin(f*(Math.PI/2));return f*f*f*f},freeze:5000})}}};var MainFrame={isMinimeExpanded:false,getJindoLibURL:function(){return"http://"+location.host+jindoAll
},setMyExpandedLayerVisible:function(a){var b=$Element("minime");if(a){b.css("background-color","#fff");b.css("position","absolute");b.css("height","410px");$Element("ad_branding_hide").show();MainFrame.isMinimeExpanded=true
}else{b.css("background-color","");b.css("position","relative");b.css("height","99px");$Element("ad_branding_hide").hide();MainFrame.isMinimeExpanded=false}},moveURL:function(a,b,c){if(!b){location.href=a;
return}window.open(a,b,c)},setNavermeNotiCnt:function(a){a=parseInt(a,10);var b=$Element($$.getSingle("#nav_hm3 a .ns"));if(a>0&&!b){b=$Element('<span class="ns"></span>');$Element($$.getSingle("#nav_hm3 a")).append(b)
}if(a==0&&b){b.leave()}else{if(a>0){b.html("<em>"+(a>99?"99+":a)+"</em>")}}}};