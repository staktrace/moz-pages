...
},this).attach("svcmore_h","mouseover");$Fn(function(b){$Element(b.currentElement).first().removeClass("over")},this).attach("svcmore_h","mouseout");this._initNewRolling();$Fn(this._onClickForServiceMenu,this).attach("fmenu","click");
...
}if(!this.bGetHTML){var b=$Ajax("/include/svcmore.html",{onload:function(e){var d=ServiceMenu;if(!d.bGetHTML){d.bGetHTML=true}else{return}$("svcmore").innerHTML=e.text();d.EventManager.init();d._initNotiRolling();
...
}a.stop()},_checkMoreButtonClass:function(a){if(!this.bGetHTML||(a==true&&!$Element("svcmore").visible())){return}if($("svcmore").className=="svcmore hide"){}else{this.Action.restoreCookieMenu()}},_openMyMenuSetLayer:function(d){this._checkMoreButtonClass();
CommonFn.closeAllHelpLayer();CommonFn.layer.toggle("svcmore");$Element("svcmore_h").toggleClass("close");var e=$Element($$.getSingle(".desc",$("svcmore")));if(e!=null&e.visible()==false){e.show()}var c=$$("#svc_story span, #svc_tools_lst span");
for(var b in c){var g=$Element(c[b]);if(g!=null&&!g.visible()){g.show("inline-block")}}var f=$Element("svc_newmenu").child(function(h){if(h._element.tagName=="A"){return true}},3);for(var b=0;b<f.length;
b++){if(f[b]._element.href==d){f[b].className("on")}else{f[b].className("")}}if(navigator.userAgent.indexOf("SHW-M180S")!=-1){if($Element("svcmore").visible()){$("ad_top").getElementsByTagName("iframe")[0].style.display="none"
}else{$("ad_top").getElementsByTagName("iframe")[0].style.display="block"}}var a=$Element("svcmore").visible();if(this.notiRoll){if(a){this.notiRoll.scrollTo(0);this.notiRoll.resume()}else{this.notiRoll.pause()
}}},_initNewRolling:function(){var a=$$("#new_service dl dd");if(a.length<=2){return}this.newRoll=new Rolling();this.newRoll.touch("new_service",{direction:1,size:(a.length-1)*16,moveSize:16,moveStep:0.1,moveInterval:30,effect:function(b){var b=Math.sin(b*(Math.PI/2));
return b*b*b*b},freeze:5000})},_initNotiRolling:function(){var a=$$("#svc_info ol li");if(a.length<2){return}this.notiRoll=new Rolling();this.notiRoll.touch("svc_info",{direction:1,size:(a.length-1)*33,moveSize:33,moveStep:0.1,moveInterval:30,effect:function(b){var b=Math.sin(b*(Math.PI/2));
return b*b*b*b},freeze:5000})},_clearCheckBox:function(){var a=this.elList;for(var b=a.length-1;b>-1;--b){a[b].checked=false;a[b].parentNode.className=""}},_initCheckBoxMenu:function(d){var a=(d)?this.aCookieList:this.aDefaultList;
var c=null;if(!this.isEqualsDefaultMenu(this.aCookieList)||!d){for(var b=a.length-1;b>-1;--b){c=$("nsvc_"+a[b]);if(c){c.checked=true;c.parentNode.className="on"}}}c=a=null},EventManager:{init:function(){$Fn(this._onClickForClose,this).attach("close_svc_layer","click");
...
}},_onClickForCheckSelectItem:function(b){var c=b.element;var a=ServiceMenu.Action;if(c.tagName=="INPUT"){if(c.checked){if(!a.addItem(c.id.split("_")[1])){b.stop()}}else{a.deleteItem(c.id.split("_")[1])
}}c=a=null;ServiceMenu.Slot.refresh()},_onClickForMyMenuSet:function(a){clickcr(a.element,"map.edit","","",a._event,1);ServiceMenu.initForm();ServiceMenu.Action.openMyMenuEdit();var d=$Element($$.getSingle(".desc",$("svcmore")));
if(d!=null&d.visible()){d.hide()}var c=$$("#svc_story span, #svc_tools_lst span");for(var b in c){var e=$Element(c[b]);if(e!=null&&e.visible()){e.hide()}}},_onClickForMenuBtns:function(a){var f=a.element;
if(f.tagName=="STRONG"){f=f.parentNode}if(f.tagName=="BUTTON"){if(f.className=="save"){clickcr(f,"edt.save","","",a._event,1);ServiceMenu.Action.saveMyMenu();var e=$$.getSingle(".mn_coin");if(e){e.href="https://bill.naver.com/pay/index.nhn"
}var g=$$.getSingle(".mn_bill");if(g){g.href="https://bill.naver.com/pay/index.nhn"}}else{if(f.className=="close"){clickcr(f,"edt.cancel","","",a._event,1);ServiceMenu.Action.restoreCookieMenu();var d=$Element($$.getSingle(".desc",$("svcmore")));
if(d!=null&d.visible()==false){d.show()}var c=$$("#svc_story span, #svc_tools_lst span");for(var b in c){var h=$Element(c[b]);if(h!=null&&!h.visible()){h.show("inline-block")}}}else{if(f.className=="return"){clickcr(f,"edt.reset","","",a._event,1);
...
CommonFn.layer.close("svcmore");if($("ad_top")){$("ad_top").getElementsByTagName("iframe")[0].style.display="block"}$("svcmore_h").focus();if(ServiceMenu.notiRoll){ServiceMenu.notiRoll.pause()}},openMyMenuEdit:function(){$Element("svc_defmenu").hide();
...
},saveMyMenu:function(){var a=ServiceMenu;var b=a.Action.areArraysEqual(a.aDefaultList,a.aCacheList);if(!b&&a.aCacheList.length>5){alert("자주 쓰는 메뉴는\n최대 5개까지만 선택 가능합니다.");a=null;return false}if(a.aCacheList.length<1){alert("선택된 메뉴가 없습니다.초기 설정으로 돌아갑니다.");
this.restoreDefalutMenu()}a.aCookieList=a.aCacheList.concat();if(a.isEqualsDefaultMenu(a.aCookieList)){$Cookie().remove("NVSERV","naver.com")}else{$Cookie().set("NVSERV",a.aCookieList.join("@"),365*10,"naver.com")
}this.closeMyMenuSet();a=null},_restoreMenu:function(h){var k=ServiceMenu;var d=(h)?k.aCookieList:k.aDefaultList;var f="<dd>"+k.serviceTemplet+"</dd>";var e=[];e.push('<dt class="blind">자주 쓰는 메뉴</dt>');
for(var g=0;g<d.length;++g){var b=d[g];var c=$("nsvc_"+b);if(!c){continue}var l=c.value.split("|");var a=l[0];var m=l[1];var j=l[2];e.push(f.replace(/#HREF/,a).replace(/#TITLE/g,j).replace(/#GDID/,m))}$("fmenu").innerHTML=e.join("");
k.aCacheList=d.concat();k=f=e=d=null},addItem:function(g){var j=ServiceMenu;var b=j.aCacheList.length;for(var f=0;f<b;f++){if(j.aCacheList[f]==g){return false}}if(j.aCacheList.length>=5){alert("자주 쓰는 메뉴는\n최대 5개까지만 선택 가능합니다.");
j=null;return false}else{$("nsvc_"+g).parentNode.className="on";var c=$("nsvc_"+g);if(!c){return false}var d=c.value.split("|");var a=d[0];var m=d[1];var h=d[2];var c=$("<dd>");var e=j.serviceTemplet;c.innerHTML=e.replace(/#GDID/g,m).replace(/#TITLE/g,h).replace(/#HREF/,a);
...
j=null;return true}},deleteItem:function(c){var b=ServiceMenu;$("nsvc_"+c).parentNode.className="";var e=$("fmenu").getElementsByTagName("a");for(var d=e.length-1;d>-1;--d){if(e[d].className=="mn_"+c){var a=e[d].parentNode;
a.parentNode.removeChild(a);a=null;break}}for(var d=b.aCacheList.length-1;d>-1;--d){if(c==b.aCacheList[d]){b.aCacheList.splice(d,1)}}b=e=null}},Slot:{html:'<dd><span class="slot"></span></dd>',show:function(){if(!$Element("svc_set_mymenu").visible()){return
...
if(c.text){this.notiElement.attr("href",c.url);this.notiElement.html(c.text)}if(!(this.aMenuTab[this.My.mode]==d&&this.My.aMyHeadlinePressList&&this.My.aMyHeadlinePressList.length==0)){NewsStand.UI.showFlickingNoti()
...
var a=NewsStand.getSortMode();if(c==NewsStand.Press.mode){b=(a=="thumb")?"nsd_all*p":"nsd_all*a"}else{if(c==NewsStand.My.mode){b=(a=="thumb")?"nsd_myn*p":"nsd_myn*a"}}return b},Press:{mode:0,noti:{text:"[알림] 언론사별 주요뉴스를 메인에서 바로 볼 수 있어요!",url:"http://news.naver.com/nboard2/read/1000003472/10000000000030004421"},init:function(){this.nPage=1;
...
}else{if(c.amigo=="Y"){this.amigoPressList.push(c)}else{if(c.viewer=="N"){this.noViewerPressList.push(c)}}}}return d}},My:{mode:1,noti:{text:"[알림] 언론사별 주요뉴스를 메인에서 바로 볼 수 있어요!",url:"http://news.naver.com/nboard2/read/1000003472/10000000000030004421"},init:function(a){this.baseHTML="<h3 class='blind'>MY뉴스</h3><div id='headline'></div>";
...
}}},_drawPressList:function(g,f){var b=new Array(a);var a=g.length;f=f||this.loadCompleted;if(a>0){for(var e=0;e<a;e++){var c=g[e];NewsStand.DataManager.loadData(c,e,a,b,f)}}else{var d=(NewsStand.aMyPressList.length>0)?this.serviceReadyHTML:this.unsubscribeHTML;
...
e.request()}},isAllLoad:function(a,c){if(a.length!=c){return false}for(var b=0;b<a.length;b++){if(a[b]==""){return false}}return true},getAllHtml:function(a){var c="";for(var b=0;b<a.length;b++){c+=a[b]
}return c}},UI:{hideFlickingNoti:function(){var a=NewsStand.UI;if(!a.oTransition){a.oTransition=new jindo.Transition()}a.oTransition.start(150,$$.getSingle("#news_contents .flick_prev"),{"@left":"-43px"},$$.getSingle("#news_contents .flick_next"),{"@right":"-43px"})
